...
async function main() {
  console.log('[Seed] Checking PostgreSQL database records...');
  ...
  // 1. AdminUser (Upsert without overwriting password if user changed it)
  ...
  // 2. PersonalInformation (Seed ONLY if empty)
  const personalCount = await prisma.personalInformation.count();
  if (personalCount === 0) {
  ...
  // 4. HeroStat (Seed ONLY if empty)
  const heroStatCount = await prisma.heroStat.count();
  if (heroStatCount === 0) {
  ...
  // 5. About & AboutHighlight (Seed ONLY if empty)
  ...
  // 6. ExperienceItem (Seed ONLY if empty)
  ...
  // 7. SkillCategory & Skill (Seed ONLY if empty)
  ...
  // 9. AcademicEducation (Seed ONLY if empty)
  const eduCount = await prisma.academicEducation.count();
  if (eduCount === 0) {
  ...
  // 10. ContactSettings (Seed ONLY if empty)
  const contactCount = await prisma.contactSettings.count();
  if (contactCount === 0) {
  ...
  // 11. SocialLink (Seed ONLY if empty)
  const socCount = await prisma.socialLink.count();
  if (socCount === 0) {
  ...
  // 12. NavigationItem (Seed ONLY if empty)
  ...
  // 13. SeoSettings (Singleton safe upsert)
  ...
  // 14. SiteSettings (Singleton safe upsert)
  ...
  // 15. PortfolioSettings (Singleton safe upsert)
  ...
  console.log('✅ [Seed Check Complete] Database records preserved cleanly!');
}
